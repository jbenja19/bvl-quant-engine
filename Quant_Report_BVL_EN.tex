...
\begin{center}
    ...
    \begin{tabular}{rl}
        \textbf{\color{darkblue}Author:}           & Jes\'{u}s Benjamin Huamaniqui Zevallos \\[4pt]
        \textbf{\color{darkblue}Institution:}     & University · Quantitative Investment Desk \\[4pt]
...
\textbf{API} & \texttt{api.py} --- FastAPI with 4h TTL cache. Endpoints: \texttt{/calculate}, \texttt{/backtest}, \texttt{/stress-test}, \texttt{/dynamic-correlation}. \\[3pt]
\textbf{Frontend} & \texttt{static/} --- Interactive Plotly.js Dashboard. 4 panels: Overview, Risk Analytics, Monte Carlo, Validation \& Stress. \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Dynamic Model and Distribution Selection}

For each asset, the engine evaluates \textbf{8 specifications} (2 volatility structures
$\times$ 4 innovation distributions) and selects the one with the lowest Bayesian Information
...
where $\hat{\ell}$ is the maximized log-likelihood, $k$ is the number of parameters, and $T$
is the number of observations. Candidate distributions: Normal, Student's t, Skewed Student's t
(Hansen, 1994), and Generalized Error Distribution (GED).
...
\begin{enumerate}[leftmargin=*]
    \item The GJR-GARCH is trained with the previous 504 days (\textasciitilde 2 years).
    \item The 1-day parametric VaR is projected using the selected distribution's quantile.
    \item It is recorded whether the actual return exceeded the estimated loss (\textit{exceedance}).
\end{enumerate}
