...
\begin{center}
    ...
    \begin{tabular}{rl}
        \textbf{\color{darkblue}Autor:}           & Jesús Benjamin Huamaniqui Zevallos \\[4pt]
        \textbf{\color{darkblue}Institución:}     & Universidad · Mesa de Inversiones Cuantitativas \\[4pt]
...
\toprule
\multicolumn{1}{c}{\textbf{Capa}} & \multicolumn{1}{c}{\textbf{Descripción}} \\
\midrule
...
\textbf{API} & \texttt{api.py} — FastAPI con caché TTL de 4 horas. Endpoints: \texttt{/api/calculate}, \texttt{/api/backtest}, \texttt{/api/stress-test}, \texttt{/api/dynamic-correlation}. \\[4pt]
...
\subsection{Selección Dinámica de Modelo y Distribución}

Para cada activo, el motor evalúa \textbf{8 especificaciones} (2 estructuras de volatilidad
$\times$ 4 distribuciones de innovaciones) y selecciona la de menor Criterio de Información
Bayesiano (BIC), que penaliza la complejidad del modelo:

\begin{equation}
    \text{BIC} = -2\,\hat{\ell} + k\,\ln(T)
\end{equation}

donde $\hat{\ell}$ es la log-verosimilitud maximizada, $k$ el número de parámetros y $T$
el número de observaciones. Distribuciones candidatas: Normal, t de Student, t de Student
Asimétrica (Hansen, 1994) y Error Generalizado (GED).
...
\begin{enumerate}[leftmargin=*]
    \item Se entrena el GJR-GARCH con los 504 días anteriores (\textasciitilde 2 años).
    \item Se proyecta el VaR paramétrico a 1 día con el cuantil de la distribución seleccionada.
    \item Se registra si el retorno real superó la pérdida estimada (\textit{exceedance}).
\end{enumerate}
...
\toprule
\multicolumn{1}{c}{\textbf{Escenario}} & \multicolumn{1}{c}{\textbf{Período}} & \multicolumn{1}{c}{\textbf{Descripción}} \\
\midrule
